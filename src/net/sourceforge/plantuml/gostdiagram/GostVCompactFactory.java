...
/**
 * GOST-specific factory that creates stadium-shaped terminators
 * instead of circles, following GOST 19.701-90 standards.
 */
public class GostVCompactFactory extends VCompactFactory {

	public GostVCompactFactory(ISkinParam skinParam, StringBounder stringBounder) {
		super(skinParam, stringBounder);
	}

	@Override
	public Ftile start(Swimlane swimlane) {
		final Style style = getSignatureCircleStart().getMergedStyle(skinParam().getCurrentStyleBuilder());
		return new FtileGostStart(skinParam(), swimlane, style);
	}

	@Override
	public Ftile stop(Swimlane swimlane) {
		final Style style = getSignatureCircleStop().getMergedStyle(skinParam().getCurrentStyleBuilder());
		return new FtileGostStop(skinParam(), swimlane, style);
	}
